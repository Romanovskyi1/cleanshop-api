...
/**
 * Миграция 005: pallets + pallet_items
 *
 * Паллета — это единица загрузки, которую клиент собирает из позиций каталога.
 * После сборки паллета назначается в truck_id.
 *
 * Бизнес-правила (проверяются на уровне сервиса):
 *   — Количество коробок в паллете ≤ product.boxes_per_pallet
 *   — Вес паллеты ≤ product.pallet_weight_kg (или дефолт 1000 кг)
 *   — Редактирование паллет разрешено только пока order.status IN ('confirmed','building')
 *
 * Триггер recalc_pallet_totals:
 *   При INSERT / UPDATE / DELETE в pallet_items автоматически
...
export class CreatePalletsAndItems1700000005000 implements MigrationInterface {
  ...
  async up(queryRunner: QueryRunner): Promise<void> {
    ...
    // ── Триггер: пересчёт totals в pallets при изменении items ───────
    await queryRunner.query(`
    ...
    // ── Триггер: пересчёт totals в orders при изменении paлlet truck ─
    await queryRunner.query(`
