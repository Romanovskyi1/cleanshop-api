...
/**
 * Dev seed — заполняет БД тестовыми данными для разработки.
 *
 * Создаёт:
 *   1 компания-клиент (CleanService sp. z o.o., PL)
 *   2 пользователя (клиент + менеджер)
 *   6 продуктов (гели, порошки, концентраты)
 *
 * Запуск: npx ts-node src/db/seeds/dev.seed.ts
 * (или через отдельный NestJS CLI command)
 */
