 ...
 * system-prompt.ts
 *
 * System prompt для ИИ-ассистента CleanShop B2B.
 * Генерируется динамически — принимает контекст клиента из БД.
 *
...
export interface ClientContext {
  companyName:     string;
  contactName:     string;
  languageCode:    string;   // ru | en | de | pl
  activeOrders:    Array<{ id: number; status: string; confirmedDate?: string }>;
  pendingPallets:  number;   // нераспределённых паллет
  pendingInvoices: number;   // инвойсов к оплате
  recentProducts:  Array<{ sku: string; name: string }>; // недавно смотрел
}

/**
 * Построить system prompt с контекстом конкретного клиента.
 */
export function buildSystemPrompt(ctx: ClientContext): string {
  ...
  const ordersContext = ctx.activeOrders.length
    ? ctx.activeOrders.map(o =>
        `- Order #${o.id}: status=${o.status}` +
        (o.confirmedDate ? `, loading date=${o.confirmedDate}` : '')
      ).join('\n')
    : 'No active orders.';
...
/**
 * Построить краткий контекст для summary (используется при эскалации).
 */
