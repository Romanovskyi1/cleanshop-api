...
export class EmailDeliveryService {
  ...
  /**
   * Отправить инвойс на email клиента.
   *
   * @param to            — email получателя
   * @param pdfBuffer     — PDF-буфер
   * @param invoiceNumber — номер инвойса
   * @param params        — данные для шаблона письма
   */
  async sendInvoice(
    to:            string,
    pdfBuffer:     Buffer,
...
<div class="wrap">
  ...
  private buildTextBody(p: {
    companyName: string; contactName: string; invoiceNumber: string;
    orderId: number; totalEur: number; due: string;
  }): string {
    const fmt = (n: number) =>
